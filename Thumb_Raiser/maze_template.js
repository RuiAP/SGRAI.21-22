...
import { playerData } from "./default_data.js";
...
export default class Maze {
	constructor(parameters) {
		function onLoad(maze, description) {
			...
			maze.initialPosition = maze.cellToCartesian(
			...
			// Create the ground
			maze.ground = new Ground({
				textureUrl: description.groundTextureUrl,
			...
			for (let i = 0; i <= description.size.width; i++) {
				...
				for (let j = 0; j <= description.size.height; j++) {
					...
					if (
						description.map[j][i] == 2 ||
						description.map[j][i] == 3
					) {
						wallObject = maze.wall.object.clone();
