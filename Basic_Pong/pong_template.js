...
export default class Pong {
    ...
    update() {
        if (this.gameRunning && !this.gamePaused) {
            ...
            if (this.ball.center.x - this.ball.radius > this.table.halfSize.x) { // Player 1 scored
                this.player1.score++; // Increment player 1 score
                this.ball.initialize();
                this.displayScore();
            }
            else if (this.ball.center.x + this.ball.radius  <  -this.table.halfSize.x) { // Player 2 scored
            ...
            if (this.player1.score >= gameData.end || this.player2.score >= gameData.end) {
                this.ball.object.visible = false; // Make the ball invisible
                this.gameRunning = false;
                this.clock.stop();
                this.displayStatus('<table>\
