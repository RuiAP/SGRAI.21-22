		...
		if (document.activeElement == document.body) {
			...
			if (event.code == this.player.keyCodes.userInterface && state) {
				...
				this.setUserInterfaceVisibility(
					!this.userInterfaceCheckBox.checked
				);
			}
	...
	mouseMove(event) {
		if (event.buttons == 1 || event.buttons == 2) {
			...
			if (
				...
				const mouseIncrement = newMousePosition
					.clone()
				...
				if (event.buttons == 1) {
					...
					if (this.changeCameraDistance) {
						...
						const width =
							this.miniMapCamera.viewport.width * windowMinSize;
						const height =
							this.miniMapCamera.viewport.height * windowMinSize;
		...
		if (!this.gameRunning) {
			...
			for (const camera of cameras) {
				this.player.object.visible =
					camera != this.firstPersonViewCamera;
