...
export default class Ball {
    ...
    update(deltaT) {
        ...
        const centerIncrement = new THREE.Vector3(x,y,z);
        ...
        if (centerIncrement.x <0) { // The ball is moving to the left
            if (this.center.x - this.radius <=  this.player1.center.x +this.player1.halfSize.x //ball has to before(in front of) the raquet
                && this.center.y <= this.player1.center.y + this.player1.halfSize.y //ball has to be bellow the top of the raquet
                && this.center.y >= this.player1.center.y - this.player1.halfSize.y //ball has to be above the bottom of the raquet
                && !(this.center.x + this.radius <= this.player1.center.x - this.player1.halfSize.x) //ball can't be after the raquet
                ) {
        ...
        /* To-do #12 - Check if the ball hit player 2 racket
            ...
            if (this.center.x + this.radius >= this.player2.center.x - this.player1.halfSize.x //ball has to before(in front of) the raquet
                && this.center.y <= this.player2.center.y + this.player1.halfSize.y //ball has to be bellow the top of the raquet
                && this.center.y >= this.player2.center.y - this.player1.halfSize.y //ball has to be above the bottom of the raquet
                && !(this.center.x - this.radius >= this.player2.center.x + this.player1.halfSize.x) //ball can't be after the raquet
                ) {
                this.direction = -this.direction + Math.PI;  // The ball rebounds
            }
        }
        ...
        */
        if (centerIncrement.y<0 &&  this.center.y <= - this.table.halfSize.y + this.radius || // The ball is moving down and hit the bottom line
            centerIncrement.y>0 && this.center.y >= this.table.halfSize.y - this.radius) { // The ball is moving up and hit the top line
